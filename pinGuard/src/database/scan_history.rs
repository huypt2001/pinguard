...
impl ScanHistory {
    ...
    pub fn add_scan_record(&self, record: &ScanHistoryRecord) -> DatabaseResult<i64> {
        ...
        let scan_modules_json = serde_json::to_string(&record.scan_modules).map_err(|e| {
            DatabaseError::SerializationError(format!("Failed to serialize scan modules: {}", e))
        })?;

        let os_info_json = serde_json::to_string(&record.os_info).map_err(|e| {
            DatabaseError::SerializationError(format!("Failed to serialize OS info: {}", e))
        })?;

        let scan_results_json = serde_json::to_string(&record.scan_results).map_err(|e| {
            DatabaseError::SerializationError(format!("Failed to serialize scan results: {}", e))
        })?;

        let config_snapshot_json = match &record.config_snapshot {
            Some(config) => Some(serde_json::to_string(config).map_err(|e| {
        ...
        info!(
            "✅ Scan history record added: {} (ID: {})",
            record.scan_id, row_id
        );
        Ok(row_id)
    }

    /// Add scan findings to detail table
    fn add_scan_findings(
        &self,
        _scan_history_id: i64,
        scan_id: &str,
        ...
        for scan_result in scan_results {
            for finding in &scan_result.findings {
                let cve_ids_json = serde_json::to_string(&finding.cve_ids).map_err(|e| {
                    DatabaseError::SerializationError(format!("Failed to serialize CVE IDs: {}", e))
                })?;

                let references_json = serde_json::to_string(&finding.references).map_err(|e| {
    ...
    pub fn get_scan_record(&self, scan_id: &str) -> DatabaseResult<Option<ScanHistoryRecord>> {
        ...
        let result = stmt.query_row(params![scan_id], |row| self.row_to_scan_record(row));
    ...
    pub fn get_recent_scans(&self, limit: u32) -> DatabaseResult<Vec<ScanHistoryRecord>> {
        ...
        let records = stmt
            .query_map(params![limit], |row| self.row_to_scan_record(row))?
    ...
    pub fn get_scans_by_hostname(
        ...
        let records = stmt
            .query_map(params![hostname], |row| self.row_to_scan_record(row))?
            .collect::<Result<Vec<_>, _>>()?;

        debug!(
            "✅ {} scan records found for hostname: {}",
            records.len(),
            hostname
        );
        Ok(records)
    ...
    pub fn get_security_score_trend(
        &self,
        hostname: &str,
        days: u32,
    ) -> DatabaseResult<Vec<SecurityScoreTrend>> {
        debug!(
            "📈 Getting security score trend: {} (last {} days)",
            hostname, days
        );
        ...
        let trends = stmt
            .query_map(params![hostname, since_date], |row| {
    ...
    pub fn get_scan_findings(&self, scan_id: &str) -> DatabaseResult<Vec<ScanFinding>> {
        ...
        let findings = stmt
            .query_map(params![scan_id], |row| {
                ...
                let cve_ids: Vec<String> = serde_json::from_str(&cve_ids_json).map_err(|e| {
                ...
                let references: Vec<String> =
                    serde_json::from_str(&references_json).map_err(|e| {
                        rusqlite::Error::SqliteFailure(
                            rusqlite::ffi::Error::new(rusqlite::ffi::SQLITE_MISMATCH),
                            Some(format!("JSON parse error: {}", e)),
                        )
    ...
    pub fn get_history_stats(&self) -> DatabaseResult<HistoryStats> {
        ...
        let mut stmt = self
            .db
            .connection()
            .prepare("SELECT COUNT(*) FROM scan_history")?;
        ...
        let mut stmt = self
            .db
            .connection()
            .prepare("SELECT COUNT(*) FROM scan_history WHERE scan_started_at >= ?1")?;
        ...
        let mut stmt = self
            .db
            .connection()
            .prepare("SELECT COUNT(DISTINCT hostname) FROM scan_history")?;
        stats.unique_hostnames = stmt.query_row([], |row| row.get(0))?;

        // Average security score
        let mut stmt = self.db.connection().prepare(
            "SELECT AVG(security_score) FROM scan_history WHERE security_score IS NOT NULL",
        )?;
        ...
        let mut stmt = self
            .db
            .connection()
            .prepare("SELECT SUM(total_findings) FROM scan_history")?;
        ...
        let mut stmt = self
            .db
            .connection()
            .prepare("SELECT scan_type, COUNT(*) FROM scan_history GROUP BY scan_type")?;
    ...
    pub fn cleanup_old_scans(&self, days_to_keep: u32) -> DatabaseResult<usize> {
        info!(
            "🧹 Cleaning up scan records older than {} days...",
            days_to_keep
        );
    ...
    fn row_to_scan_record(&self, row: &Row) -> rusqlite::Result<ScanHistoryRecord> {
        ...
        let scan_modules: Vec<String> = serde_json::from_str(&scan_modules_json).map_err(|e| {
            rusqlite::Error::SqliteFailure(
                rusqlite::ffi::Error::new(rusqlite::ffi::SQLITE_MISMATCH),
                Some(format!("JSON parse error: {}", e)),
            )
        ...
        let os_info: OsInfo = serde_json::from_str(&os_info_json).map_err(|e| {
            rusqlite::Error::SqliteFailure(
                rusqlite::ffi::Error::new(rusqlite::ffi::SQLITE_MISMATCH),
                Some(format!("JSON parse error: {}", e)),
            )
        ...
        let scan_results: Vec<ScanResult> =
            serde_json::from_str(&scan_results_json).map_err(|e| {
                rusqlite::Error::SqliteFailure(
                    rusqlite::ffi::Error::new(rusqlite::ffi::SQLITE_MISMATCH),
                    Some(format!("JSON parse error: {}", e)),
                )
        ...
        let scan_type = scan_type_str.parse::<ScanType>().map_err(|e| {
            rusqlite::Error::SqliteFailure(
                rusqlite::ffi::Error::new(rusqlite::ffi::SQLITE_MISMATCH),
...
mod tests {
    ...
    use crate::scanners::{ScanMetadata, ScanResult, ScanStatus};
