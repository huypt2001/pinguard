...
impl ReportManager {
    ...
    pub fn generate_report(
        ...
        let filename = output_filename
            .unwrap_or_else(|| self.generate_default_filename(&report.metadata.report_id, format));
    ...
    pub fn generate_multi_format_report(
        ...
        let base_name = base_filename
            .unwrap_or_else(|| format!("pinGuard-report-{}", report.metadata.report_id));
        ...
        if generated_files.is_empty() {
            Err(ReportError::RenderingError(
                "Failed to generate any reports".to_string(),
            ))
        } else {
            Ok(generated_files)
        }
    ...
    pub fn print_report_summary(&self, report: &SecurityReport) -> Result<(), ReportError> {
        ...
        println!(
            "System: {} ({})",
            report.metadata.system_info.hostname, report.metadata.system_info.os_version
        );
        ...
        println!(
            "Security Score: {}/100 ({})",
            report.summary.security_score, report.summary.risk_level
        );
        ...
        println!(
            "Scanners: {}/{} successful",
            report.summary.successful_scans, report.summary.total_scans
        );
    ...
    pub fn print_detailed_statistics(&self, report: &SecurityReport) -> Result<(), ReportError> {
        ...
        println!(
            "  • Fastest: {}",
            report.statistics.scan_performance.fastest_scanner
        );
        println!(
            "  • Slowest: {}",
            report.statistics.scan_performance.slowest_scanner
        );
        println!(
            "  • Total Items: {}",
            report.statistics.scan_performance.total_items_scanned
        );
    ...
    pub fn print_format_info(&self) {
        ...
        let formats = [
            (
                ReportFormat::Json,
                "JSON",
                "Machine-readable data format, perfect for integration",
            ),
            (
                ReportFormat::Html,
                "HTML",
                "Interactive web report with charts and filtering",
            ),
        ...
        for (format, name, description) in formats.iter() {
            let marker = if format == &self.default_format {
                "👉"
            } else {
                "  "
            };
...
mod tests {
    ...
    fn test_report_manager_creation() {
        let manager =
            ReportManager::new(Some("test_reports".to_string()), Some(ReportFormat::Html));
    ...
    fn test_quick_report_generation() {
        ...
        test_manager
            .set_output_directory(test_dir.to_string())
            .unwrap();
    ...
    fn test_multi_format_generation() {
        ...
        let result = test_manager.generate_multi_format_report(
            &report,
            formats,
